...
namespace Hast.Communication.Helpers
{
    ...
    internal static class EthernetCommunicationHelpers
    {
        ...
        public static async Task<UdpReceiveResult> UdpSendAndReceiveAsync(byte[] datagram, int bindingPort, IPEndPoint targetEndpoint, int receiveTimeoutMilliseconds)
        {
            ...
            // We need two UDP clients for sending and receiving datagrams. See: http://stackoverflow.com/questions/221783/udpclient-receive-right-after-send-does-not-work/222503#222503
            using (var receiverClient = CreateUdpClient(udpEndpoint))
            using (var senderClient = CreateUdpClient(udpEndpoint))
            {
                var receiveTask = receiverClient.ReceiveAsync(receiveTimeoutMilliseconds);
        ...
        public static async Task<IEnumerable<UdpReceiveResult>> UdpSendAndReceiveAllAsync(byte[] datagram, int bindingPort, IPEndPoint targetEndpoint, int receiveTimeoutMilliseconds)
        {
            ...
            using (var senderClient = CreateUdpClient(udpEndpoint))
            {
                var receiveTask = receiverClient.ReceiveAllAsync(receiveTimeoutMilliseconds);
                var sendTask = senderClient.SendAsync(datagram, datagram.Length, targetEndpoint);
