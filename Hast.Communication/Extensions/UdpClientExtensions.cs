...
namespace System.Net.Sockets
{
    ...
    {
        /// <summary>
        /// Receive UDP datagram if there is one in present or wait one until the timeout expires. In that case a default object will return.
        /// </summary>
        /// <param name="receiveTimeoutMilliseconds">Timeout within the client needs to wait for an UDP datagram.</param>
        /// <returns>Returns with the datagram if received or a default value if the timeout has expired.</returns>
        ...
        /// <summary>
        /// Receives all the UDP datagrams coming simultaneously. Stops receiving when the timeout has expired after the last datagram.
        /// </summary>
        /// <param name="receiveTimeoutMilliseconds">Timeout within the client needs to wait for a single datagram.</param>
        /// <returns>Returns with the datagram list.</returns>
        public static async Task<IEnumerable<UdpReceiveResult>> ReceiveAllAsync(this UdpClient client, int receiveTimeoutMilliseconds)
        {
            ...
            while (read)
            {
                ...
                if (result != default(UdpReceiveResult))
                    datagramList.Add(result);
                else
                    read = false;
            }
