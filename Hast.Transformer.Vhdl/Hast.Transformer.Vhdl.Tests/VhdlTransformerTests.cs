...
namespace Hast.Transformer.Vhdl.Tests
{
    ...
    public class VhdlTransformerTests
    {
        ...
        public async Task CallProxyWellStructured()
        {
            ...
            Assert.That(ifElseElement.True is InlineBlock && ((InlineBlock)ifElseElement.True).Body.Count == 1 && ((InlineBlock)ifElseElement.True).Body.Single() is Case, "The structure of the call proxy's inner branching is not correct.");

            var caseElement = (Case)((InlineBlock)ifElseElement.True).Body.Single();
