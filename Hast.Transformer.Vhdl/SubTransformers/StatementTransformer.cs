...
namespace Hast.Transformer.Vhdl.SubTransformers
{
    public class StatementTransformer : IStatementTransformer
    {
        ...
        private void TransformInner(Statement statement, SubTransformerContext context)
        {
            ...
            switch (statement)
            {
                case VariableDeclarationStatement variableStatement:
                    TransformVariableStatement(variableStatement, context);
                    return;
                ...
                case ReturnStatement returnStatement:
                    TransformReturnStatement(returnStatement, context);
                    return;
                case IfElseStatement ifElse:
                    TransformIfElse(ifElse, context);
                    return;
                ...
                case WhileStatement whileStatement:
                    TransformWhileStatement(whileStatement, context);
                    return;
                case ThrowStatement:
                ...
                case SwitchStatement switchStatement:
                    TransformSwitchStatement(switchStatement, context);
                    return;
                ...
                case BreakStatement:
                    ...
                    throw new NotSupportedException(
                        "Break statements outside of switch statements and loops are not supported."
                            .AddParentEntityName(statement));
                case GotoStatement gotoStatement:
                    currentBlock.Add(
                        stateMachine.CreateStateChange(scope.LabelsToStateIndicesMappings[gotoStatement.Label]));
                ...
                case LabelStatement labelStatement:
                    ...
                    currentBlock.ChangeBlockToDifferentState(
                        stateMachine.States[labelStateIndex].Body,
                        labelStateIndex);
                    return;
                case EmptyStatement:
                    // Nothing to do.
        ...
        private void TransformVariableStatement(VariableDeclarationStatement variableStatement, SubTransformerContext context)
        {
            ...
            var isTaskFactory = variableSimpleType?.Identifier == nameof(TaskFactory);
            ...
            if (!omitStatement)
            {
                foreach (var variableInitializer in variableStatement.Variables)
                {
                    stateMachine.LocalVariables.Add(new Variable
                    {
                        ...
                        DataType = _declarableTypeCreator.CreateDeclarableType(
                            variableInitializer,
                            variableType,
        ...
        private void TransformSwitchStatement(SwitchStatement switchStatement, SubTransformerContext context)
        {
            ...
            var afterCaseStateBlock = new InlineBlock(
                new GeneratedComment(vhdlGenerationOptions =>
                    ...
                    StateNameGenerator(caseStartStateIndex, vhdlGenerationOptions, stateMachine) +
                    "."));
        ...
        private void TransformWhileStatement(WhileStatement whileStatement, SubTransformerContext context)
        {
            ...
            string WhileStartStateIndexNameGenerator(IVhdlGenerationOptions vhdlGenerationOptions) =>
                vhdlGenerationOptions.NameShortener(stateMachine.CreateStateName(currentBlock.StateMachineStateIndex));
            ...
            var lastState = stateMachine.States[stateMachine.States.Count - 1].Body;
        ...
        private void TransformIfElse(IfElseStatement ifElse, SubTransformerContext context)
        {
            ...
            var afterIfElseStateBlock = new InlineBlock(
                new GeneratedComment(vhdlGenerationOptions =>
                    ...
                    StateNameGenerator(ifElseStartStateIndex, vhdlGenerationOptions, stateMachine) +
                    "."));
            ...
            ifElseCommentsBlock.Add(new GeneratedComment(vhdlGenerationOptions =>
                "    * The true branch starts in state " + StateNameGenerator(trueStateIndex, vhdlGenerationOptions, stateMachine) +
                " and ends in state " + StateNameGenerator(trueEndStateIndex, vhdlGenerationOptions, stateMachine) + "."));
            if (falseStateIndex != 0)
            {
                ifElseCommentsBlock.Add(new GeneratedComment(vhdlGenerationOptions =>
                    "    * The false branch starts in state " + StateNameGenerator(falseStateIndex, vhdlGenerationOptions, stateMachine) +
                    " and ends in state " + StateNameGenerator(falseEndStateIndex, vhdlGenerationOptions, stateMachine) + "."));
            }

            ifElseCommentsBlock.Add(new GeneratedComment(vhdlGenerationOptions =>
                "    * Execution after either branch will continue in the following state: " +
                StateNameGenerator(afterIfElseStateIndex, vhdlGenerationOptions, stateMachine) + "."));
        ...
        private static string StateNameGenerator(
            int index,
